...
// ---------------------------------------------------------------------------
// ML / deep-learning tab — classification metrics, embeddings, XAI, training.
// Every panel is a composed ML builder (addConfusionMatrix / addEmbedding / …)
// over the same shared WebGL2 context. Synthetic, in-browser data.
// ---------------------------------------------------------------------------
function buildML(grid: HTMLElement): void {
  reseed();
  ...
  // 7 — Decision boundary (field + training points)
  {
    ...
    for (let iy = 0; iy < ny; iy++) for (let ix = 0; ix < nx; ix++) {
      ...
      values[iy * nx + ix] = 1 / (1 + Math.exp(-(1.6 - (x * x + y * y)) * 2.2));
    }
    const M = 220;
