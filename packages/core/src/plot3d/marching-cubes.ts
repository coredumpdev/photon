...
// Which of the 12 edges are crossed for each of the 256 corner-sign combinations.
const EDGE_TABLE = [
...
// Triangles (edge triplets, -1 terminated) for each of the 256 cases.
const TRI_TABLE = [
...
// The two corners each of the 12 edges connects (Bourke corner numbering).
...
export function marchingCubes(
  ...
  const wx = (x: number) => ex[0] + (x / Math.max(1, nx - 1)) * (ex[1] - ex[0]);
  const wy = (y: number) => ey[0] + (y / Math.max(1, ny - 1)) * (ey[1] - ey[0]);
  const wz = (z: number) => ez[0] + (z / Math.max(1, nz - 1)) * (ez[1] - ez[0]);
  const idx = (x: number, y: number, z: number) => x + y * nx + z * nx * ny;
  const val = (x: number, y: number, z: number) => values[idx(x, y, z)]!;
  // Central-difference gradient (points toward increasing value; negate for outward normal).
  ...
  const edgeP: [number, number, number][] = new Array(12);
  const edgeN: [number, number, number][] = new Array(12);
    ...
    for (let y = 0; y < ny - 1; y++) {
      for (let x = 0; x < nx - 1; x++) {
        ...
        for (let e = 0; e < 12; e++) {
          ...
          // Interpolate gradients at the two corners, then negate for an outward normal.
