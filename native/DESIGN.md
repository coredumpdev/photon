...
**Ported the other way**: the zoom bounds. Both cores used to let a wheel zoom
run until the numbers gave out. A few thousand notches out takes the span past
1e130, and because zooming about the cursor preserves that point it preserves
the rounding error in it too — around 1e117 by then; when the span comes back
down to something readable, both ends have rounded onto the same value and the
chart is blank for good. The mirror image happens zooming in, below about an eps
of the view's own magnitude.

The Qt gallery is what surfaced it, so the fix was written here first:
`Plot::zoom_fits` bounds a zoom-out at a billion times the data's extent and a
zoom-in at 1e-14 of the view's magnitude — both far outside any readable chart,
both leaving the way back intact. It then went back to the web core as
`zoomFits` in `plot.ts`, with the same two constants and the same reasoning, so
the two remain in parity. This is the first fix to travel in that direction.
