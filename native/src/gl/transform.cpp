...
PixelTransform PixelTransform::of(int width, int height) {
  PixelTransform t;
  const float w = width > 0 ? static_cast<float>(width) : 1.0f;
  const float h = height > 0 ? static_cast<float>(height) : 1.0f;
  t.sx = 2.0f / w;
  t.ox = -1.0f;
  // The layout measures y downward from the top; GL's clip space measures it
  // upward from the middle. Hence the negative scale.
  t.sy = -2.0f / h;
  t.oy = 1.0f;
  return t;
}
